...
const Modal: React.FC<ModalProps> = ({
  ...
  // 모달 배경
  const modalBackground = `fixed inset-0 bg-blue-700 bg-opacity-30 transition-opacity duration-300 z-40 ${
    isModalOpen ? 'opacity-50' : 'opacity-0 pointer-events-none'
  }`;

  // 모달 컨테이너
  const modalContainer = `fixed bottom-0 left-0 right-0 md:bottom-0 left-0 right-0 top-0 flex items-end md:items-center justify-center z-50 transform transition-all duration-300 ${
  ...
  return (
    <>
      <div className={modalBackground} onClick={onCloseModal}></div>
